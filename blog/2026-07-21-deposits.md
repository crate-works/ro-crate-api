...
1. **Create** — `POST /deposits` opens a deposit for a new RO-Crate
   ...
   existing one.
2. **Stage** — `PUT /deposit/{id}/metadata` stages the metadata document
   (full replace)
   and `PUT /deposit/{id}/file/{fileId}` stages files, either inline or via
   a presigned upload target, in any order.
